...
app.post("/api/identity", async (req: Request, res: Response) => {
  try {
    ...
    const contactWithEmail = contacts.find((contact) => contact.email === email && contact.linkPrecedence === "primary")
    const contactWithPhoneNumber = contacts.find((contact) => contact.phoneNumber === phoneNumber && contact.linkPrecedence === "primary")
    ...
    if(contactWithEmail && contactWithPhoneNumber) {
      ...
      res.status(200).json({
        "contacts": {
          ...
          "phoneNumbers": [contactWithEmail.phoneNumber, contactWithPhoneNumber.phoneNumber],
    ...
    if(contacts.length === 1 && contacts[0].linkPrecedence === "secondary") {
      ...
      else if(phoneNumber !== contacts[0].phoneNumber && email === contacts[0].email) {
        const newContact = await prisma.contact.create({
          data: {
            email,
            phoneNumber,
    ...
    console.log("details:", isEmailExist, isPhoneNumberExists, primaryContactId)
